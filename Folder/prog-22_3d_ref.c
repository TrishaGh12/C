/*
* Name	 : Trisha Ghosh
* Prog-22: C program to implement three-dimensional reflection with respect to a pivot point. Output depends upon user’s choice. [Use OpenGL Library]
...
void translate(float tx,float ty,float tz)
{
	for(int i=0;i<n;i++)
	{
		input[i][0]=input[i][0]+tx;
		input[i][1]=input[i][1]+ty;
		input[i][2]=input[i][2]+tz;
	}
}
void Reflection()
{
