/*
    ...
    Program Name : Write a program in C to implement two-dimensional scaling with respect to a pivot point. Output depends upon user’s choice. [Use OpenGL Library]
    Name :Trisha Ghosh
...
void Init();
void Draw();
void draw_pixelFirst(int x , int y); // This finction will draw the geometric shape before scaling
void draw_pixelSecond(int x , int y); // This function will draw the geometric shape after scaling
...
double **mat , Xf , Yf , Sx , Sy; // Xf and Yf are the scaling parameter and theta is the angle of the scaling
...
int main(int argc , char **argv)
{
	...
	printf("Enter the scaling parameter and the angle for scaling :\n");
	printf("Xf and Yf  :");
	scanf("%lf %lf",&Xf , &Yf );
	printf("Sx and Sy  :");
	scanf("%lf %lf",&Sx , &Sy );
...
{
	glColor3f(1, 0, 0);
    	double newX =  (x*Sx) + (Xf*(1-Sx));
    	double newY = (y*Sy) + (Yf*(1-Sy));
    	glBegin(GL_POINTS);
